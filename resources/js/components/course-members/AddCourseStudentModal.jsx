...
export default function AddCourseStudentModal( { courseId, onAddMemberDone, onHileModal }) {

    const onSubmit = (event) => {
        event.preventDefault();
        let data = event.target.elements;
        var login_name = data.login_name.value;
        axios.post(`course/${courseId}/add-student/${login_name}`)
        .then((res)=>{
    ...
    return (
        <Modal
            title={<span><i className="fa fa-group color-primary">Add student to course</i></span>}
            visible={true}
            footer={null}
